...
import {append} from "../utils/createJSON.js";
import getMatkul from "../utils/getMatkul.js";
import toDateID from "../utils/toDateID.js";

export default async function() {
  // ambil semua data list matkul
  let listMatkul = await getMatkul();
  listMatkul =
      listMatkul.map((data) => { return {id : data.id, nama : data.matkul}; });
  // mengambil id dan nama matkul
  const listMatkulID = listMatkul.map((matkul) => matkul.id);
  const menuMatkul =
      listMatkul.map((matkul) => { return ` [${matkul.id}]  ${matkul.nama}`; })
          .join("\n");
...
`);
  ...
  while (true) {
    ...
    let isAnswerValid =
        readline.question("Ingin menambahkan deskripsi lagi?(y/n)   : ")
            .toLowerCase();
    while (!allowToInput.includes(isAnswerValid)) {
      isAnswerValid =
          readline.question("Inputan kamu salah, lanjut atau tidak?(y/n)   : ")
              .toLowerCase();
    }
  ...
  > Deadline\t: ${tugas.date}
  > Deskripsi\t: \n${
      desc.map((text, id) => { return `\t${id + 1}.  ${text}\n`; }).join("")}
`);
